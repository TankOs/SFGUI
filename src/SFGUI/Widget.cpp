...
void Widget::GrabFocus( Ptr widget ) {
	if( !m_parent.lock() ) {
		...
		m_parent.lock()->GrabFocus( widget );
	}
}
...
void Widget::SetParent( Widget::Ptr parent ) {
	...
	if( m_parent.lock() ) {
		Container::Ptr  oldparent( boost::shared_dynamic_cast<Container>( m_parent.lock() ) );
...
Widget::HandleEventResult Widget::HandleEvent( const sf::Event& event ) {
	...
	case sf::Event::MouseMoved: {
		...
		if( GetAllocation().Contains( static_cast<float>( event.MouseMove.X ), static_cast<float>( event.MouseMove.Y ) ) ) {
			...
			if( m_mouse_in == false ) {
				...
				if( m_parent.lock() ) {
					m_parent.lock()->RegisterEventHook( sf::Event::MouseMoved, shared_from_this() );
				}
			}
			...
			if( !m_drag_info && m_parent.lock() ) {
				m_parent.lock()->UnregisterEventHook( sf::Event::MouseMoved, shared_from_this() );
			}
	...
	case sf::Event::MouseButtonPressed: {
		...
		if( m_mouse_button_down == -1 ) {
			if( m_mouse_in ) {
				...
				if( m_parent.lock() ) {
					m_parent.lock()->RegisterEventHook( sf::Event::MouseButtonReleased, shared_from_this() );
				}
	...
	case sf::Event::MouseButtonReleased: {
		...
		if( m_mouse_button_down == event.MouseButton.Button ) {
			...
			if( m_parent.lock() ) {
				m_parent.lock()->UnregisterEventHook( sf::Event::MouseButtonReleased, shared_from_this() );
			}
