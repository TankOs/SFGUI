	...
	if( orientation == Scale::Orientation::HORIZONTAL ) {
		...
		queue->Add(
			Renderer::Get().CreateRect(
				sf::FloatRect(
					{ slider_rect.width / 2.f, ( scale->GetAllocation().height - trough_thickness ) / 2.f },
					{ scale->GetAllocation().width - slider_rect.width, trough_thickness }
				),
				trough_color
			)
		);
		...
		queue->Add(
			Renderer::Get().CreateRect(
				sf::FloatRect(
					{ ( scale->GetAllocation().width - trough_thickness ) / 2.f, slider_rect.height / 2.f },
					{ trough_thickness, scale->GetAllocation().height - slider_rect.height }
				),
				trough_color
			)
		);
